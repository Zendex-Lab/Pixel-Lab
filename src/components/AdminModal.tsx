...
import { X, Shield, RefreshCcw, Save, Paintbrush, AlertTriangle, Loader2 } from "lucide-react";
...
export default function AdminModal({
  ...
  const [fillStatus, setFillStatus] = useState<{ msg: string; type: "success" | "error" } | null>(null);
  ...
  const runFill = async () => {
    ...
    setFillStatus(null);
    try {
      const affected = await pixelService.adminFillArea(minX, minY, maxX, maxY, fillColorIdx);
      ...
      <div className="glass-strong w-full max-w-md p-6 rounded-3xl border border-red-500/30 shadow-[0_0_40px_rgba(220,38,38,0.15)] relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-1 mb-5 p-1 rounded-xl bg-[var(--glass-bg)] border border-[var(--glass-border)]">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all ${
              tab === "stats" ? "bg-red-500/15 text-red-400" : "text-[var(--muted-foreground)] hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all ${
              tab === "moderation" ? "bg-red-500/15 text-red-400" : "text-[var(--muted-foreground)] hover:text-white"
            }`}
          >
        ...
        {tab === "stats" && (
          <div className="space-y-4">
            {status && (
              <div className={`p-3 rounded-xl text-xs font-semibold ${status.type === "success" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
                {status.msg}
              </div>
            )}
        ...
        {tab === "moderation" && (
          <div className="space-y-4">
            <p className="text-xs text-[var(--muted-foreground)]">
              Заливка прямоугольной области холста. Без учёта зарядов и кулдауна. Координаты: X от 0 до {gridWidth - 1}, Y от 0 до {gridHeight - 1}.
            </p>

            {fillStatus && (
              <div className={`p-3 rounded-xl text-xs font-semibold ${fillStatus.type === "success" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
                {fillStatus.msg}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs text-[var(--muted-foreground)] font-medium">X от</label>
                <input
                  type="number"
                  value={minX}
                  onChange={(e) => { setMinX(clamp(Number(e.target.value), 0, gridWidth - 1)); resetConfirm(); }}
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500/50"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs text-[var(--muted-foreground)] font-medium">X до</label>
                <input
                  type="number"
                  value={maxX}
                  onChange={(e) => { setMaxX(clamp(Number(e.target.value), 0, gridWidth - 1)); resetConfirm(); }}
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500/50"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs text-[var(--muted-foreground)] font-medium">Y от</label>
                <input
                  type="number"
                  value={minY}
                  onChange={(e) => { setMinY(clamp(Number(e.target.value), 0, gridHeight - 1)); resetConfirm(); }}
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500/50"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs text-[var(--muted-foreground)] font-medium">Y до</label>
                <input
                  type="number"
                  value={maxY}
                  onChange={(e) => { setMaxY(clamp(Number(e.target.value), 0, gridHeight - 1)); resetConfirm(); }}
                  className="w-full bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl px-4 py-2.5 text-sm outline-none focus:border-red-500/50"
                />
              </div>
            </div>
            ...
            {!boundsValid && (
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium">
                <AlertTriangle className="h-3.5 w-3.5 shrink-0" /> "От" не может быть больше "До"
              </div>
            )}

            <div className="space-y-1.5">
              <label className="text-xs text-[var(--muted-foreground)] font-medium">Цвет заливки</label>
              <div className="grid grid-cols-8 gap-2">
                {paletteHex.map((hex, idx) => (
                  <button
                    key={hex + idx}
                    onClick={() => { setFillColorIdx(idx); resetConfirm(); }}
                    title={hex}
                    className={`aspect-square rounded-lg border-2 transition-all ${
                      fillColorIdx === idx ? "border-red-400 scale-110" : "border-[var(--glass-border)] hover:scale-105"
                    }`}
            ...
            <div className="flex items-center justify-between text-xs text-[var(--muted-foreground)] px-1">
              ...
              <span className="font-semibold text-[var(--foreground)]">{boundsValid ? cellCount : "—"}</span>
            </div>
            ...
            >
              {isFilling ? (
                ...
                <>
                  <AlertTriangle className="h-4 w-4" /> Точно? Нажмите ещё раз ({cellCount} ячеек)
                </>
              ) : (
                <>
