...
import { X, Shield, RefreshCcw, Save } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 p-4">
      <div className="glass-strong w-full max-w-md p-6 rounded-3xl border border-red-500/30 shadow-[0_0_40px_rgba(220,38,38,0.15)] relative">
        ...
        {status && (
          <div
            className={`mb-4 p-3 rounded-xl text-xs font-semibold ${status.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}
          >
            {status.msg}
          </div>
        )}
